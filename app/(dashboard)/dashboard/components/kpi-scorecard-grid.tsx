...
function ScorecardCard({ item }: { item: ScorecardItem }) {
  ...
  return (
    <article className="flex h-full min-h-0 flex-col rounded-lg border border-slate-100 bg-slate-50/50 p-2 dark:border-slate-800 dark:bg-slate-900/40">
      <div className="flex items-start justify-between gap-1.5">
        <div className="min-w-0 flex-1">
          <h4 className="text-[11px] font-semibold leading-tight text-slate-800 dark:text-slate-100">
            {item.metric}
          </h4>
          <p className="mt-0.5 line-clamp-1 text-[9px] text-slate-500 dark:text-slate-400">
            Goal · {item.target}
          </p>
        </div>
      ...
      <div className="mt-2 flex items-end justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="truncate text-base font-bold tabular-nums text-slate-900 dark:text-slate-50">
            {item.actual}
          </p>
        </div>
