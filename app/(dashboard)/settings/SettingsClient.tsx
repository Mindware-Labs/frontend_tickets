...
export function SettingsClient() {
  const [activeSection, setActiveSection] = useState<SectionKey>("dispositions");
  ...
  const handleAdd = () => {
    if (activeSection === "dispositions") setEditTarget({ kind: "disposition" });
    else if (activeSection === "ticketTypes") setEditTarget({ kind: "ticketType" });
  ...
  return (
    <div className="px-3 pt-2 pb-8 lg:px-5">
      ...
      {/* Main panel */}
      <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)] dark:border-slate-800 dark:bg-slate-950">
        <div
        ...
        <div className="flex flex-col gap-2 border-b border-slate-100 px-3.5 py-2.5 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
          <div className="flex w-full rounded-lg border border-slate-200/80 bg-slate-100 p-1 sm:w-auto sm:min-w-[280px] dark:border-slate-800 dark:bg-slate-900/80">
            {SECTIONS.map((tab) => (
              <button
                key={tab.key}
                type="button"
                ...
                className={cn(
                  ...
                  activeSection === tab.key
                    ? "bg-white font-semibold text-[#008f68] shadow-sm dark:bg-slate-950 dark:text-emerald-300"
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1 sm:flex-none">
              ...
              <input
                ...
                className="h-8 w-full rounded-lg border border-transparent bg-slate-50 pl-8 pr-7 text-xs text-slate-700 placeholder:text-slate-400 hover:border-slate-300 focus:border-[#008f68] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#008f68]/20 sm:w-52 dark:bg-slate-900 dark:text-slate-200"
              />
              {query ? (
                <button
                  type="button"
...
>({
  ...
  return (
    <div>
      {/* Column header */}
      <div className="flex items-center gap-2.5 border-b border-slate-100 bg-slate-50/80 px-3.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:border-slate-800 dark:bg-slate-900/60">
    ...
    >
      <span
        className={cn(
          ...
          active ? "bg-[#008f68]" : "bg-slate-300 dark:bg-slate-600",
        )}
        aria-hidden
      />
      {active ? "Active" : "Off"}
